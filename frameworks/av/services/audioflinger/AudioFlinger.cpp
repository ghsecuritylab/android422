...
void AudioFlinger::PlaybackThread::dumpTracks(int fd, const Vector<String16>& args)
{
    ...
    fprintf(fd, "Normal mixer raw underrun counters: partial=%u empty=%u\n",
...
void AudioFlinger::PlaybackThread::dumpInternals(int fd, const Vector<String16>& args)
{
    ...
    result.append(buffer);
    write(fd, result.string(), result.size());
    fprintf(fd, "Fast track availMask=%#x\n", mFastTrackAvailMask);
...
#define TEE_SINK_READ 1024
            ...
            write(teeFd, &temp, sizeof(temp));
            close(teeFd);
            fprintf(fd, "FastMixer tee copied to %s\n", teePath);
        } else {
            fprintf(fd, "FastMixer unable to create tee %s: \n", strerror(errno));
        }
    }
