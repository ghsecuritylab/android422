        ...
        egl_connection_t* cnx, uint32_t mask)
{
    ...
    const char* const search1 = "%s/vendor/lib/egl/lib%s_%s.so";
    const char* const search2 = "%s/system/lib/egl/lib%s_%s.so";
	const char* out = getenv("ANDROID_PRODUCT_OUT");

    snprintf(driver_absolute_path, PATH_MAX, search1, out, kind, tag);
    if (access(driver_absolute_path, R_OK)) {
        snprintf(driver_absolute_path, PATH_MAX, search2, out, kind, tag);
        if (access(driver_absolute_path, R_OK)) {
